...
async function speakIt(raw){
  ...
  if (typeof window.elevenSpeak === 'function') {
    try {
      ...
      // Eleven failed — log and fall through to next provider below
      try { console.warn('[speakIt] ElevenLabs failed, falling back:', elevenResult); } catch(e) {}
    } catch(e) {
      activateWaves(false);
    }
  }
...
function kRender(){
  ...
  if(kPhonics){
    ...
    if (kCat === 'numbers') {
      ...
      // Big digit tile — tapping it should say the WORD, not the digit
      // (because saying the digit "1" out loud already sounds like "one" — but
      // we want the kid to learn that ONE is the spoken form of the symbol 1)
      let html = `<button class="kz-ltr" style="background:#fde68a;font-size:1.4em;font-weight:900;" onclick="speakIt('${k.w.replace(/'/g,"\\'")}')" aria-label="Tap to hear ${k.w}">${digit}</button>`;
      // Then one tile per letter of the word (e.g. O, N, E)
