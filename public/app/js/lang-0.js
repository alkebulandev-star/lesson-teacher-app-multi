  ...
  observer = new MutationObserver(function(mutations){
    ...
    mutations.forEach(function(m){
      m.addedNodes.forEach(function(node){
        // Skip if the addition is inside an area we don't translate
        var checkEl = node.nodeType === 1 ? node : node.parentElement;
        if (!checkEl) return;
        if (!isTranslatableElement(checkEl)) return;
        if (node.nodeType === 1){
          scheduleTreeApply(node);
        } else if (node.nodeType === 3){
          if (node.parentElement) scheduleTreeApply(node.parentElement);
        }
      });
      // Don't observe characterData — when AI streams a lesson, the
      // text node mutates char-by-char and translating mid-stream
      // produces broken output.
    });
  });
  // Note: characterData:false — see comment above.
...
var LTLang = {
  ...
  set: function(lang, opts){
    ...
    if (lang === 'en'){
      ...
      // Estimate work: count translatable text nodes that aren't cached
      var pending = 0;
