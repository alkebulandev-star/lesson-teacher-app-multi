...
async function callEleven(apiKey: string, voiceId: string, text: string, languageCode?: string) {
  ...
  try {
    ...
    // Hint the model toward the right language. Only set when caller asks.
    // Valid codes for v2 include 'en','yo','ig','ha' plus many others.
