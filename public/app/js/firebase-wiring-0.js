...
function isDemoMode(){
  try {
    if (window.LT_DEMO_MODE) return true;
    var qs = new URLSearchParams(location.search);
    if (qs.get('demo') === '1' || qs.get('demo') === 'true') {
      try { sessionStorage.setItem('lt_demo', '1'); } catch(e){}
      return true;
    }
    if (sessionStorage.getItem('lt_demo') === '1') return true;
  } catch(e){}
  return false;
}
...
// ────────────────────────────────────────────────────────────────
// 3. Role gating: students can't enter Parent Hub by default
// ────────────────────────────────────────────────────────────────
function patchRoleGating(){
  ...
  window.goTo = function(id){
    if (isDemoMode()) return origGoTo.apply(this, arguments);
    var role = (window._LT_LAST_PROFILE && window._LT_LAST_PROFILE.role) || null;
    var profile = window._LT_LAST_PROFILE || null;

    // Block students from Parent Hub
    if (id === 'pg-parent' && role === 'student'){
      showRoleToast('The Parent Hub is for parents only. Add ?demo=1 to the URL to unlock everything for testing.');
      return origGoTo.call(this, 'pg-landing');
    }

    // For signed-in students, intercept "go to level picker" → go straight to classroom
    if (id === 'pg-beta' && role === 'student' && profile && profile.section && profile.classLevel){
      try {
    ...
    // For signed-in parents, "Enter Classroom" → Parent Hub (or classroom if they prefer; Parent Hub fits better)
